...
in
{
  ...
  services.nginx = {
    ...
    virtualHosts."${adguardDomain}" = {
      # We use the DNS-01 ACME cert defined in security.acme
      enableACME = false; 
      forceSSL = true;

      # Point to the certificate files generated by ACME
      sslCertificate = "${acmeAdguardDomainDir}/full.pem";
      sslCertificateKey = "${acmeAdguardDomainDir}/key.pem";

      # Listen on the VPN IP (10.0.0.2) on port 443 (HTTPS)
      listen = [
        {
          addr = "10.0.0.2";
          port = 443;
      ...
      locations."/" = {
        # Proxy requests to AdGuard Home's web interface, which is listening on 127.0.0.1:3080
